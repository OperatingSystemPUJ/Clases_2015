 ...
 * The C library function int atexit(void (*func)(void)) causes the specified function func to be 
 * called when the program terminates. You can register your termination function anywhere you like, 
 ...
 * Run in background and then perform: ps aux | grep fork 
 * 
 * Zombies are DEAD processes. They can not be 'kill' (You cannot kill the DEAD). 
 * All processes eventually die, and when they do they become zombies.
 * But, a Zombie process is a process state when the child dies before the parent process.
 ...
 * (look at PPID displayed by ps -l). You have three choices: Fix the parent process (make it wait); 
