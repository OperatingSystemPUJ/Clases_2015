...

{
    ...
    error = sqlite3_prepare_v2(conn,
        ...
        "select card_number,amount from requests order by card_number where status=0",
    ...
    while (sqlite3_step(res) == SQLITE_ROW){
        ...
        error = sqlite3_prepare_v2(conn,
            ...
            "select card_number,cash from bd order by card_number 

            where card_number=\'sqlite3_column_text(res, 0)\'", 1000, &res1, &tail);
        ...
        if(sqlite3_column_text(res1, 1)>=sqlite3_column_text(res, 1)){
            ...
            error = sqlite3_exec(conn,
                         ...
                         "update db set cash=cash-\'sqlite3_column_text(res, 1)\' 

                         where card_number=sqlite3_column_text(res, 0)",
